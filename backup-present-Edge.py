...
def move_window_to_display(hwnd, display_index):
    ...
    print(f"已将窗口移动到显示器 {display_index} 并设置为全屏")
    ...
    # 打印窗口的当前坐标和大小
    print(f"窗口的当前坐标和大小:")
    print(f"  X: {window_x}, Y: {window_y}, Width: {window_width}, Height: {window_height}")

    # 最大化窗口
    win32gui.ShowWindow(hwnd, win32con.SW_MAXIMIZE)
    print(f"已将窗口最大化")

def start_edge_on_display(display_index, url):
    """
    启动 Edge 并移动到指定的显示器。
    :param display_index: 显示器索引（0 表示主显示器，1 表示扩展显示器）
    """
    ...
    # 启动 Edge 浏览器
    try:
        edge_process = subprocess.Popen([edge_path, "--new-window", url])
        #edge_process = subprocess.run(["start", edge_path, url], shell=True)
...
start_edge_on_display(1, "http://localhost:3030/")

start_edge_on_display(0, "http://localhost:3030/presenter/")